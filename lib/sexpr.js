...
import { sat, many, some, char, choice, Do } from './Parser.js'
...
const [list, pair] = (function () {
    ...
    const list = Do(function* () {
        const paren = yield openParen.rcomma(many(whitespace))
        const res = yield many(sexpr.rcomma(many(whitespace)))
        yield close[paren].rcomma(many(whitespace))
        return res
    })
