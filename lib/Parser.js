...
export const sepBy = (s, p) => Do(function* () {
    const first = yield p;
    const rest = yield s.comma(sepBy1(s, p)).alt(of([]))

    return [first, ...rest]

}).alt(of([]))

const sepBy1 = (s, p) => sepBy(s, p).chain(xs => xs.length != 0 ? of(xs) : empty);
...
export const whitespace = sat(x => /\s/.test(x))

export const choice = (...ps) => ps.reduce((accu, p) => accu.alt(p))

// We use arrays as a representation of Maybe values
export const optional = (px) => of(x => [x]).ap(px).alt(of([]))
